...
parser.add_argument("--unfoldings", type=int, dest="unfoldings", help="Number of unfolded", default=24)
parser.add_argument("--patch_size", type=int, dest="patch_size", help="Size of image blocks to process", default=96)
parser.add_argument("--e_prox", type=float, default=0.02, help='')
parser.add_argument("--rescaling_init_val", type=float, default=1.0)
parser.add_argument("--lmbda_prox", type=float, default=0.02, help='intial threshold value of ista')
...
parser.add_argument("--beta_prox", type=float, default=0.02, help='intial threshold value of ista')
parser.add_argument("--mu_prox", type=float, default=0.02, help='')
parser.add_argument("--gamma_1", type=float, default=0.02, help='')
parser.add_argument("--gamma_2", type=float, default=0.02, help='')
parser.add_argument("--l_prox", type=float, default=0.02, help='')

#data
parser.add_argument("--model_name", type=str, dest="model_name", help="The name of the model to be saved.", default=None)
parser.add_argument("--test_path", type=str, help="Path to the dir containing the testing datasets.", default="D:/0_Hyperspectral/deepl/test/")

parser.add_argument("--tqdm", type=str2bool, default=False)
parser.add_argument("--gt_path", type=str, help="Path to the dir containing the ground truth datasets.", default="D:/0_Hyperspectral/deepl/test/")
...
model=init_model(kernel_size=args.kernel_size,num_filters=args.num_filters,unfoldings=args.unfoldings,lmbda_prox=args.lmbda_prox,stride=args.stride,multi_theta=args.multi_theta,verbose=args.verbose, beta=args.beta_prox,mu=args.mu_prox, e=args.e_prox, gamma_1=args.gamma_1, gamma_2=args.gamma_2,l=args.l_prox)
...
for batch,fname in tqdm(loader,disable=not args.tqdm):
    ...
    with torch.set_grad_enabled(False):
        ...
        if device_name == 'cpu':
            ...
            scio.savemat(args.out_dir +  fname + 'Res.mat', {'output': output.squeeze(0).detach().cpu().numpy()})
        if torch.cuda.is_available():
        ...
        num_iters += 1
        tac1 = time.time()
        tqdm.write(f'psnr avg {psnr_batch} ssim avg {ssim_batch} sam avg {sam_batch},  fsim avg {fsim_batch}, ergas avg {ergas_batch}, time avg {(tac1-tic1)}, max gpu mem allocated {max_mem:0.1f} Mb  ')
tac = time.time()
